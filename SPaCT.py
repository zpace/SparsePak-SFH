...
Some tools for correcting un-flux-calibrated SparsePak data when there's no standard star.

Performance not guaranteed. Success dependent on correct redshift, correct centering of SparsePak & SDSS.
...
def z_test(im, fiberflat, sdss, fiber = 47, dz = 0.):
	...
	Arguments:
	 ...
	 - dz (0.): redshift correction wrt SDSS (I recommend testing redshifts by hand before generating a correction)
	...
	sparsepak_wavelength = 1. / (1. + dz) * np.linspace(CRVAL1, CRVAL1 + CDELT1 * NAXIS1, NAXIS1, endpoint = True )
...
def sdss_cal(im, fiberflat, sdss, dz, z = 0, verbose = False, fiber = 47, blur = 100, full_output = False):
	...
	Arguments:
	 ...
	 - z: optional (but recommended) parameter that lets you check wavelength solution with known Balmer lines
	...
	if verbose:
		...
		ax4 = plt.subplot(111)
		ax4.plot(sparsepak_wavelength, np.polyval(corr_poly, sparsepak_wavelength) * fluxcal * interp_corr(im, fiberflat)[fiber], c = 'b', linewidth = 1., label = 'SDSS-flux-calibrated SparsePak spectrum')
		ax4.plot(sdss_wavelength[sdss_wavelength < 6500.], sdss_spectrum[sdss_wavelength < 6500.], linewidth = 0.25, c = 'r', label = 'SDSS spectrum')
		...
		if z:
			...
			for line in balmer:
				...
				ax4.annotate('H-' + line[0], xy = (line[1], 0.9*ax4.get_ylim()[1]), xytext = (line[1]+10., 0.95*ax4.get_ylim()[1]), size = 14)
			metals = [ ['Mg[I]', 5175.], ['Ca[I]', 4307.], ['Ca H', 3968.], ['Ca K', 3933.] ]
			for line in metals:
				line_c = cmodels.irgb_string_from_xyz(ciexyz.xyz_from_wavelength(line[1]/10.))
				ax4.axvline(line[1] * (1 + dz + z), color = line_c)
				ax4.annotate(line[0], xy = (line[1], 0.9*ax4.get_ylim()[1]), xytext = (line[1]+10., 0.95*ax4.get_ylim()[1]), size = 14)
		plt.show()
...
def write_corr_frame(ifu_corr, im, z, dz, objname, verbose = False):
	...
	Arguments:
	 ...
	 - dz: correct z offset found by trial-and-error (must be the same as the one plugged into sdss_cal)
	...
	sparsepak_wavelength = np.linspace(CRVAL1, CRVAL1 + CDELT1 * NAXIS1, NAXIS1, endpoint = True )
	...
	if verbose == True: print fits.open(objname + '_fluxcal.fits')[0].header
