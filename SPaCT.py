...
def z_test(im, sdss, z = 0., test = False):
	'''
	...
	print 'NAXIS1:', NAXIS; print 'CRVAL1:', CRVAL1; print 'CDELT1:', CDELT1
	...
	#plot the SparsePak spectrum
	plt.plot(wavelength, fluxcal * interp_corr(im)[47], linewidth = 0.5, label = 'Interpolation-corrected')

	# plot the SDSS spectrum
	plt.plot(sdss.wavelength(), sdss.spectrum()/np.max(sdss.spectrum)/np.percentile(fluxcal * interp_corr(im)[47], 98.), linewidth = 0.5, color = 'grey', label = 'SDSS Spectrum', zorder = 0)
