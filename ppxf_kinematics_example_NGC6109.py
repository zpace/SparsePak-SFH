...
def ppxf_kinematics_example_sdss():
    ...
    #
    galaxy = np.asarray( t[(l['Wavelength'] > 3540) * (l['Wavelength'] > 7409)]/np.median(t[(l['Wavelength'] > 3540) & (l['Wavelength'] > 7409)]) )  # Normalize spectrum to avoid numerical issues
    wave = np.asarray( l['Wavelength'][(l['Wavelength'] > 3540) * (l['Wavelength'] < 7409)] )
    print(wave)
